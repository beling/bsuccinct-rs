...
pub trait KeySet<K> {
    ...
    fn retain_keys_into_vec<F, P, R>(mut self, mut filter: F, mut retained_earlier: P, mut retained_count: R) -> Vec<K>
        where F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize, Self: Sized, K: Clone
    {
        let new_len = retained_count();
        self.retain_keys(&mut filter, &mut retained_earlier, || new_len);
        self.map_each_key(|k| (*k).clone(), new_len, |k| retained_earlier(k) && filter(k))
    }

    /// Works like `retain_keys_with_indices` and converts `self` into the vector of retained keys.
    fn retain_keys_with_indices_into_vec<IF, F, P, R>(mut self, index_filter: IF, mut filter: F, mut retained_earlier: P, mut retained_count: R) -> Vec<K>
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize, Self: Sized, K: Clone
    {
        let new_len = retained_count();
        self.retain_keys_with_indices(index_filter, &mut filter, &mut retained_earlier, || new_len);
        self.map_each_key(|k| (*k).clone(), new_len, |k| retained_earlier(k) && filter(k))
    }

    /// Works like `par_retain_keys` and converts `self` into the vector of retained keys.
    fn par_retain_keys_into_vec<F, P, R>(mut self, filter: F, retained_earlier: P, retained_count: R) -> Vec<K>
        where F: Fn(&K) -> bool + Sync + Send, P: Fn(&K) -> bool + Sync + Send,
              R: Fn() -> usize, Self: Sized, K: Clone + Send
    {
        let new_len = retained_count();
        self.par_retain_keys(&filter, &retained_earlier, || new_len);
        self.par_map_each_key(|k| (*k).clone(), new_len, |k| retained_earlier(k) && filter(k))
    }

    /// Works like `par_retain_keys_with_indices` and converts `self` into the vector of retained keys.
    fn par_retain_keys_with_indices_into_vec<IF, F, P, R>(mut self, index_filter: IF, filter: F, retained_earlier: P, retained_count: R) -> Vec<K>
        where IF: Fn(usize) -> bool + Sync + Send,  F: Fn(&K) -> bool + Sync + Send,
              P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize, Self: Sized, K: Clone + Send
    {
        let new_len = retained_count();
        self.par_retain_keys_with_indices(index_filter, &filter, &retained_earlier, || new_len);
        self.par_map_each_key(|k| (*k).clone(), new_len, |k| retained_earlier(k) && filter(k))
    }
}
...
impl<K: Sync + Send> KeySet<K> for Vec<K> {
    ...
    #[inline(always)] fn retain_keys_with_indices<IF, F, P, R>(&mut self, mut index_filter: IF, _filter: F, _retained_earlier: P, _retained_count: R)
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        let mut index = 0;
    ...
    fn retain_keys_into_vec<F, P, R>(mut self, filter: F, retained_earlier: P, retained_count: R) -> Vec<K>
        where F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize, Self: Sized, K: Clone
    {
        self.retain_keys(filter, retained_earlier, retained_count);
        self
    ...
    /// Works like `retain_keys_with_indices` and converts `self` into the vector of retained keys.
    fn retain_keys_with_indices_into_vec<IF, F, P, R>(mut self, index_filter: IF, filter: F, retained_earlier: P, retained_count: R) -> Vec<K>
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize, Self: Sized, K: Clone
    {
        self.retain_keys_with_indices(index_filter, filter, retained_earlier, retained_count);
        self
...
impl<'k, K: Sync + Send + Clone> KeySet<K> for SliceSourceWithClones<'k, K> {
    ...
    #[inline(always)] fn map_each_key<R, M, P>(&self, map: M, len_hint: usize, _retained_hint: P) -> Vec<R>
        where M: FnMut(&K) -> R, P: FnMut(&K) -> bool
    {
        if let Some(ref retained) = self.retained {
    ...
    #[inline(always)] fn retain_keys_with_indices<IF, F, P, R>(&mut self, mut index_filter: IF, _filter: F, retained_earlier: P, retained_count: R)
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        let mut index = 0;
...
impl<'k, K: Sync> KeySet<K> for SliceSourceWithRefs<'k, K> {
    ...
    fn retain_keys<F, P, R>(&mut self, mut filter: F, _retained_earlier: P, _retained_count: R)
        where F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
    ...
    fn par_retain_keys<F, P, R>(&mut self, filter: F, _retained_earlier: P, _retained_count: R)
        where F: Fn(&K) -> bool + Sync + Send, P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize
    {
        if let Some(ref mut r) = self.retained {
            let slice = self.slice; // fixes "cannot borrow `self` as immutable because it is also borrowed as mutable"
    ...
    fn retain_keys_with_indices<IF, F, P, R>(&mut self, mut index_filter: IF, _filter: F, _retained_earlier: P, _retained_count: R)
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        let mut index = 0;
        if let Some(ref mut r) = self.retained {
...
{
    fn into_cache<F, P, R>(self, filter: F, retained_earlier: P, retained_count: R) -> Self
        where F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize {
        match self {
            Self::Dynamic(dynamic_key_set, _) => Cached(dynamic_key_set.retain_keys_into_vec(filter, retained_earlier, retained_count)),
            Self::Cached(_) => self
        }
    }

    fn par_into_cache<F, P, R>(self, filter: F, retained_earlier: P, retained_count: R) -> Self
        where F: Fn(&K) -> bool + Sync + Send, P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize
    {
        match self {
            Self::Dynamic(dynamic_key_set, _) => Cached(dynamic_key_set.par_retain_keys_into_vec(filter, retained_earlier, retained_count)),
            Self::Cached(_) => self
        }
    }

    fn into_cache_with_indices<IF, F, P, R>(self, index_filter: IF, filter: F, retained_earlier: P, retained_count: R) -> Self
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        match self {
            Self::Dynamic(dynamic_key_set, _) => Cached(dynamic_key_set.retain_keys_with_indices_into_vec(index_filter, filter, retained_earlier, retained_count)),
            Self::Cached(_) => self
        }
    }

    fn par_into_cache_with_indices<IF, F, P, R>(self, index_filter: IF, filter: F, retained_earlier: P, retained_count: R) -> Self
        where IF: Fn(usize) -> bool + Sync + Send, F: Fn(&K) -> bool + Sync + Send,
              P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize
    {
        match self {
            Self::Dynamic(dynamic_key_set, _) => Cached(dynamic_key_set.par_retain_keys_with_indices_into_vec(index_filter, filter, retained_earlier, retained_count)),
            Self::Cached(_) => self
        }
    }
}
...
impl<K: Clone + Sync + Send, KS: KeySet<K>> KeySet<K> for CachedKeySet<K, KS>
{
    ...
    fn retain_keys<F, P, R>(&mut self, filter: F, retained_earlier: P, mut retained_count: R)
        where F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        match self {
            Self::Dynamic(key_set, clone_threshold) => {
                ...
                if len < *clone_threshold {
                    *self = mem::take(self).into_cache(filter, retained_earlier, || len)
                    //*self = Cached(key_set.retain_keys_into_vec(filter, retained_earlier, retained_count))
                } else {
                    key_set.retain_keys(filter, retained_earlier, || len)
                }
            },
        ...
        where F: Fn(&K) -> bool + Sync + Send, P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize
    {
        match self {
            Self::Dynamic(key_set, clone_threshold) => {
                let len = retained_count();
                if len < *clone_threshold {
                    *self = mem::take(self).par_into_cache(filter, retained_earlier, || len)
                } else {
                    key_set.par_retain_keys(filter, retained_earlier, || len)
                }
            },
    ...
    fn retain_keys_with_indices<IF, F, P, R>(&mut self, index_filter: IF, filter: F, retained_earlier: P, mut retained_count: R)
        where IF: FnMut(usize) -> bool, F: FnMut(&K) -> bool, P: FnMut(&K) -> bool, R: FnMut() -> usize
    {
        match self {
            Self::Dynamic(key_set, clone_threshold) => {
                let len = retained_count();
                if len < *clone_threshold {
                    *self = mem::take(self).into_cache_with_indices(index_filter, filter, retained_earlier, || len)
                } else {
                    key_set.retain_keys_with_indices(index_filter, filter, retained_earlier, || len)
                }
            },
            Self::Cached(v) => v.retain_keys_with_indices(index_filter, filter, retained_earlier, retained_count)
        }
    }
              ...
              P: Fn(&K) -> bool + Sync + Send, R: Fn() -> usize
    {
        match self {
            Self::Dynamic(key_set, clone_threshold) => {
                let len = retained_count();
                if len < *clone_threshold {
                    *self = mem::take(self).par_into_cache_with_indices(index_filter, filter, retained_earlier, || len)
                } else {
                    key_set.par_retain_keys_with_indices(index_filter, filter, retained_earlier, || len)
                }
            },
