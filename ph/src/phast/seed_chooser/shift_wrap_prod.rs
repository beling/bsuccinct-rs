...
fn shift_only_wrapped_bucket_evaluator_m1(bits_per_seed: u8, slice_len: u16) -> [i32; 7] {
    match (bits_per_seed, slice_len) {
        ...
        (8, ..=512) => [-68807, 84278, 109684, 123741, 133881, 142307, 148696], // 4.1, slice=512, W=512
        (8, ..=1024) => [-17933, 212784, 282214, 311888, 332059, 348404, 359350],  // 4.1, slice=1024, W=512
        (_, ..=512) => [-49623, 70161, 121132, 147329, 163658, 177855, 186663],  // 5.1, slice=512, zmiana o 0.01%
        (9, ..=1024) => [-51179, 70664, 124513, 150002, 167975, 182188, 190787],  // 5.1, slice=1024, W=512
        (..=9, ..=2048) => [-1515, 8677, 183680, 239832, 272523, 301189, 318292],  // 9, 5.1, slice=2048, W=512
        (10, ..=1024) => [-4303, 15341, 80122, 86853, 107050, 119702, 130341], // 5.7, slice=1024, W=512
        (..=10, ..=2048) => [-2645, 14141, 109926, 154341, 183008, 208608, 223324],   // 10, 5.7, slice=2048, W=512
        (11, ..=1024) => [-1807, 30889, 90893, 123659, 149236, 153463, 160949],    // 6.3, slice=1024, W=512 USELESS
        (11, ..=2048) => [-1602, 16372, 110223, 151205, 181644, 207718, 222776],  // 6.3, slice=2048, W=512
        (11, _) => [-5604, 8223, 9638, 107658, 157090, 208931, 233747], // 6.3, slice=4096, W=512, bit smaller than slice=2048
        (_, ..=1024) => [-2200, 30526, 114151, 141825, 162473, 178078, 184304], // 12, 6.8, slice=1024, W=512 USELESS
        (_, ..=2048) => [-1363, 13520, 138018, 187090, 224974, 252901, 264812], // 12, 6.8, slice=2048, W=512 USELESS
        (_, _) => [-3918, 38101, 29549, 53294, 97784, 148857, 173841], // 12, 6.8, slice=4096, W=512
    }
}
