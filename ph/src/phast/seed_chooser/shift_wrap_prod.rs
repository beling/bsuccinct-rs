...
fn shift_only_wrapped_bucket_evaluator_m1(bits_per_seed: u8, slice_len: u16) -> [i32; 7] {
    ...
    match (bits_per_seed, slice_len) { // WINDOW_SIZE = 512
        ...
        (8, ..=512) => [0, 155766, 180245, 193932, 204551, 212586, 218698], // W=512, S=8, 4.1, slice=512, 2.69%
        (8, ..=1024) => [0, 209215, 280825, 309695, 327493, 344871, 356812],  // W=512, S=8, 4.1, slice=1024, 2.89%
        (..=8, ..=2048) => [0, 26139, 271653, 347300, 392285, 428332, 451371],  // W=512, S=8, 4.1, slice=2048, 4.19%
        (_, ..=512) => [0, 117052, 147627, 161781, 173003, 178616, 188289],  // W=512, S=9, 5.1, slice=512, 3.32%
        ...
        (10, ..=1024) => [0, 21976, 73786, 97751, 117815, 129270, 140449], // W=512, S=10, 5.7, slice=1024, 2.11%
        (10, ..=2048) => [0, 14266, 109751, 155766, 183250, 209803, 224626],   // W=512, S=10, 5.7, slice=2048, 1.61%
        (..=10, ..=4096) => [0, 271, 66094, 192896, 237575, 291651, 321291],   // W=512, S=10, 5.7, slice=4096, 2.68%
        (11, ..=1024) => [0, 26410, 90209, 120740, 139066, 154391, 161283],    // W=512, S=11, 6.3, slice=1024, 3.25%
        (11, ..=2048) => [0, 24166, 111992, 156652, 186630, 212961, 227787],  // W=512, S=11, 6.3, slice=2048, 1.29%
        (11, ..=4096) => [0, 182, 28989, 118002, 178027, 223753, 249037], // W=512, S=11, 6.3, slice=4096, 1.14%
        (11, _) => [0, 17, 165005, 253419, 311596, 352199, 377737], // W=512, S=11, 6.3, slice=8192, 1.10%
        (_, ..=1024) => [0, 33929, 111765, 144460, 164900, 181227, 187164], // W=512, S=12, 6.8, slice=1024, 4.97%
        (_, ..=2048) => [0, 10631, 123398, 182611, 216249, 245834, 257782],   // W=512, S=12, 6.8, slice=2048, 2.11%
        (_, _) => [0, 39386, 41110, 65140, 127482, 175702, 200631], // W=512, S=12, 6.8, slice=4096 and 8192, 0.73%
    }
        ...
        match (bits_per_seed, slice_len) {
            ...
            (11, _) => [0, 17900, 19784, 40648, 89345, 132760, 159319], // W=256, S=11, 6.3, slice=4096, 1.24%
            ...
            (_, _) => [0, 40496, 30478, 46656, 80691, 115102, 139934], // W=256, S=12, 6.8, slice=4096, 0.74%
        }
    }
