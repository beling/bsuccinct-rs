    ...
    fn bucket_evaluator(&self, bits_per_seed: u8, slice_len: u16) -> Weights {
        Weights(match MULTIPLIER {
            ...
            _ => match (bits_per_seed, slice_len) { // multiplier=3, almost the same result as for multiplier=2 weights
                ...
                (10, ..=1024) => [-2394, 29640, 81921, 108732, 126229, 141102, 150457], // 5.7, slice=1024
                (..=10, _) => [-3417, 13564, 81208, 133035, 168506, 198114, 214382],   // 10, 5.7, slice=2048
                (11, ..=1024) => [-1555, 25982, 126717, 155711, 174202, 191358, 198247],    // 6.3, slice=1024
                (11, _) => [-2229, 21208, 88554, 137643, 169905, 200075, 213746],  // 6.3, slice=2048
                (_, ..=1024) => [-2206, 33628, 110901, 143147, 161228, 177559, 183794], // 12, 6.8, slice=1024
                (_, _) => [-2665, 16252, 98048, 149519, 183487, 214959, 227347],   // 12, 6.8, slice=2048
            }
        })
    }
