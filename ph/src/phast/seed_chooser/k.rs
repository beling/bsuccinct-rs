...
fn prod_for(k: u16, bits_per_seed: u8, slice_len: u16) -> (&'static (u16, [i32; 7], ProdOfValuesKEval), Option<&'static (u16, [i32; 7], ProdOfValuesKEval)>) {
    let values = match (bits_per_seed, slice_len) {
        ...
        (..=7, _) => PROD_S7_L1024.as_ref(),
        (8, ..=32) => PROD_S8_L32.as_ref(),
        (8, ..=64) => PROD_S8_L64.as_ref(),
        (8, ..=128) => PROD_S8_L128.as_ref(),
        (8, ..=256) => PROD_S8_L256.as_ref(),
        (8, ..=512) => PROD_S8_L512.as_ref(),
        (8, ..=1024) => PROD_S8_L1024.as_ref(),
        (8, _) => PROD_S8_L2048.as_ref(),
        (9.., ..=1024) => PROD_S9_L1024.as_ref(),
        (9.., _) => PROD_S9_L2048.as_ref(),
    };
...
/*const PROD_S7_L64: [(u16, [i32; 7], ProdOfValuesKEval); 11] = [   // for W=512 15M keys
    (2, [0, 11434, 15588, 18313, 19728, 21685, 22467], P{value_shift: 0.014315, free_shift: 1.751797, first_weight: 0.278672})  // 2.30% for 3.7 λ=5.66
];*/
const PROD_S7_L128: [(u16, [i32; 7], ProdOfValuesKEval); 12] = [   // for W=512 15M keys S8 params work excellent for k=16, 32; and OK+ for k=48, 8, 4, 2, weak for k=64, 100
    ...
    (3, [0, 5634, 6889, 9063, 12726, 15074, 16268], P{value_shift: 0.018230, free_shift: 1.566620, first_weight: 0.539185}), // 1.30% for 3.7 λ=7.42
    ...
    (5, [0, 24642, 25835, 26111, 31060, 31076, 31901], P{value_shift: 0.008258, free_shift: 1.680940, first_weight: 0.738092}), // 1.05% for 3.7 λ=10.63
    (6, [0, 49614, 51018, 51052, 54281, 54567, 55083], P{value_shift: 0.007186, free_shift: 1.454377, first_weight: 0.806906}), // 0.95% for 3.7 λ=12.13
    (8, [0, 113540, 116621, 118681, 121124, 122568, 122921], P{value_shift: 0.004061, free_shift: 1.383542, first_weight: 0.815084}), // 0.82% for 3.7 λ=15.03
    (10, [0, 127186, 136995, 142077, 144494, 146156, 146416], P{value_shift: 0.004077, free_shift: 1.379174, first_weight: 0.828238}), // 0.73% for 3.7 λ=17.80
...
const PROD_S7_L256: [(u16, [i32; 7], ProdOfValuesKEval); 12] = [   // for W=512 15M keys S8 params work excellent for k=32, 16, 8, 4; and OK+ for k=48, 2, OK k=64
    ...
    (3, [0, 57690, 57723, 57730, 61169, 61650, 62561], P{value_shift: 0.001991, free_shift: 1.924222, first_weight: 0.954355}), // 1.49% for 3.7 λ=7.42
    ...
    (5, [0, 62577, 63718, 65387, 73065, 73339, 74346], P{value_shift: 0.004076, free_shift: 1.290662, first_weight: 0.206234}), // 0.85% for 3.7 λ=10.63
    (6, [0, 85141, 85467, 87195, 91809, 91833, 92577], P{value_shift: 0.004932, free_shift: 1.269037, first_weight: 0.264518}), // 0.78% for 3.7 λ=12.13
    ...
    (10, [0, 162391, 192513, 197130, 199932, 202690, 202975], P{value_shift: 0.004254, free_shift: 1.272361, first_weight: 0.872906}), // 0.62% for 3.7 λ=17.80
...
const PROD_S7_L512: [(u16, [i32; 7], ProdOfValuesKEval); 14] = [   // for W=512 15M keys  S8 params work excellent for k=64, 48, 32, 16, 8; and OK for k=4, 2
    ...
    (3, [0, 58629, 78656, 87299, 104375, 115511, 118653], P{value_shift: 0.015128, free_shift: 1.493484, first_weight: 0.130663}), // 0.86% for 3.7 λ=7.42
    ...
    (5, [0, 76823, 77762, 77955, 86045, 89542, 90913], P{value_shift: 0.005116, free_shift: 1.382913, first_weight: 0.410752}),  // 0.77% for 3.7 λ=10.63
    (6, [0, 101349, 108885, 111226, 117920, 121353, 122410], P{value_shift: 0.004703, free_shift: 1.356749, first_weight: 0.568669}), // 0.72% for 3.7 λ=12.13
    (7, [0, 47869, 51741, 52445, 56211, 56874, 57569], P{value_shift: 0.005019, free_shift: 1.277399, first_weight: 0.667103}), // 0.67% for 3.7 λ=13.60
    ...
    (9, [0, 156407, 188776, 192060, 195103, 198028, 198302], P{value_shift: 0.003667, free_shift: 1.225008, first_weight: 0.888079}), // 0.62% for 3.7 λ=16.42
    (10, [0, 162481, 192193, 198146, 200992, 203815, 204083], P{value_shift: 0.003703, free_shift: 1.218465, first_weight: 0.875453}), // 0.61% for 3.7 λ=17.80
    ...
    (32, [0, 180272, 199878, 210486, 212787, 215426, 215696], P{value_shift: 0.003654, free_shift: 1.134473, first_weight: 0.914314}), // 0.61% for 3.7 λ=44.60
...
const PROD_S7_L1024: [(u16, [i32; 7], ProdOfValuesKEval); 6] = [
    ...
    (6, [0, 109432, 141958, 148774, 154139, 157776, 159327], P{value_shift: 0.005361, free_shift: 1.322542, first_weight: 0.513551}), // 0.73% for 3.7 λ=12.13
    ...
    (10, [0, 131871, 181437, 212190, 213330, 214398, 214429], P{value_shift: 0.003292, free_shift: 1.199457, first_weight: 0.844621}), // 0.67% for 3.7 λ=17.80
];

const PROD_S8_L32: [(u16, [i32; 7], ProdOfValuesKEval); 2] = [   // for W=512 10M keys
...
const PROD_S8_L64: [(u16, [i32; 7], ProdOfValuesKEval); 4] = [   // for W=512 10M keys
...
const PROD_S8_L128: [(u16, [i32; 7], ProdOfValuesKEval); 17] = [   // for W=512 10M keys
    (2, [0, 6793, 11759, 15111, 18087, 21041, 22155], P{value_shift: 0.013562, free_shift: 1.844853, first_weight: 0.600869}), // 1.43% for 4.3 λ=6.58
...
const PROD_S8_L256: [(u16, [i32; 7], ProdOfValuesKEval); 19] = [   // for W=512 10M keys
    (2, [0, 57700, 57731, 57734, 61171, 61652, 62563], P{value_shift: 0.001943, free_shift: 1.966776, first_weight: 0.954382}), // 1.64% for 4.3 λ=6.58
    ...
    (5, [0, 51543, 53207, 53392, 55952, 56385, 57110], P{value_shift: 0.003041, free_shift: 1.412530, first_weight: 0.961096}), // 0.73% for 4.3 λ=12.35
    (6, [0, 95054, 102115, 106025, 108433, 109543, 110008], P{value_shift: 0.003301, free_shift: 1.326613, first_weight: 0.918017}), // 0.67% for 4.3 λ=14.10
    ...
    (16, [0, 162393, 197268, 211386, 213179, 215109, 215261], P{value_shift: 0.004013, free_shift: 1.275321, first_weight: 0.871179}), // 0.45% for 4.3 λ=29.77 20M
    ...
    (64, [0, 171614, 191795, 211173, 212673, 214404, 214480], P{value_shift: 0.004615, free_shift: 1.471455, first_weight: 0.886417}), // 0.20% for 4.2 λ=89.60 70M
    (100, [0, 165504, 182741, 195792, 196969, 198722, 198788], P{value_shift: 0.005175, free_shift: 1.717621, first_weight: 0.921655}), // 0.14% for 4.1 λ=127.23 100M keys
    ...
    (1000, [0, 153668, 169151, 175170, 178768, 183912, 183945], P{value_shift: 0.005486, free_shift: 1.902363, first_weight: 0.669649}), // 2.45% for 4.3 λ=655.35
];
const PROD_S8_L512: [(u16, [i32; 7], ProdOfValuesKEval); 16] = [   // for W=512 10M keys
...
const PROD_S8_L1024: [(u16, [i32; 7], ProdOfValuesKEval); 7] = [   // for W=512 10M keys
    ...
    (5, [0, 144044, 201409, 214665, 221014, 225087, 226181], P{value_shift: 0.004140, free_shift: 1.232151, first_weight: 0.559441}), // 1.09% for 4.5 λ=12.92
...
const PROD_S8_L2048: [(u16, [i32; 7], ProdOfValuesKEval); 6] = [   // for W=512 10M keys
    ...
    (4, [0, 81201, 95830, 96831, 124094, 149918, 153043], P{value_shift: 0.006220, free_shift: 1.326950, first_weight: 0.274944}), // 0.90% for 4.4 λ=10.77
    (5, [0, 90470, 112538, 118540, 135380, 151775, 153325], P{value_shift: 0.005613, free_shift: 1.226365, first_weight: 0.427567}), // 0.90% for 4.4 λ=12.64
...
const PROD_S9_L1024: [(u16, [i32; 7], ProdOfValuesKEval); 6] = [   // for W=512 10M keys
    ...
    (3, [0, 106512, 112212, 117445, 127477, 143930, 146016], P{value_shift: 0.005462, free_shift: 1.224635, first_weight: 0.150164}), // 0.95% for 5.1 λ=10.22
    ...
    (16, [0, 136707, 189846, 226851, 243604, 258998, 264526], P{value_shift: 0.003044, free_shift: 3.105925, first_weight: 0.785002}), // 0.99% for 5.1 λ=35.31 20M
];
const PROD_S9_L2048: [(u16, [i32; 7], ProdOfValuesKEval); 8] = [   // for W=512 10M keys
    ...
    (3, [0, 96735, 121967, 141941, 171730, 201019, 204636], P{value_shift: 0.006377, free_shift: 1.311951, first_weight: 0.191832}), // 0.98% for 5.1 λ=10.22
    ...
    (6, [0, 112252, 149639, 170317, 178752, 186382, 186425], P{value_shift: 0.004912, free_shift: 1.144739, first_weight: 0.568293}), // 0.98% for 5.1 λ=16.73
    ...
    (9, [0, 143784, 197851, 232034, 232467, 232873, 232899], P{value_shift: 0.003244, free_shift: 1.187008, first_weight: 0.821200}), // 1.08% for 5.1 λ=22.64
    (10, [0, 134372, 189460, 225676, 226023, 226349, 226376], P{value_shift: 0.003278, free_shift: 1.216569, first_weight: 0.830729}), // 1.12% for 5.1 λ=24.53
    (16, [0, 136763, 191986, 224518, 224715, 224891, 224905], P{value_shift: 0.003326, free_shift: 1.153749, first_weight: 0.847241}), // 1.20% for 5.1 λ=35.31 20M
];