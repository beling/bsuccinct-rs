    ...
    pub fn new(bits_per_seed: u8, slice_len: u16) -> Self {
        Self(if slice_len <= 256 {  // this is used only for small number of keys
            ...
            match (bits_per_seed, slice_len) {
                ...
                (8, ..=1024) => [-51774, 74183, 128085, 157456, 179060, 196306, 206203],    // 8, 4.4, 1024, W=512
                (..=8, _) => [-2419, 19625, 137589, 195995, 232036, 261678, 281232], // 8, 4.4, 2048, W=512, 0.79%
                //(..=8, _) => [1842, -17481, 148915, 270544, 344295, 393666, 432618], // 8, 4.4, 4096, W=512, 0.83% WRONG
                (9, ..=512) => [-60668, 86903, 117046, 132208, 140749, 149552, 153428], // 5.3
                (9, ..=1024) => [-59075, 64370, 122529, 152009, 169865, 184948, 191697],   // 5.3, W=512
                (9, ..=2048) => [-2341, 15197, 143019, 208073, 245150, 269831, 282233],  // 9, 5.3, 2048, W=512 (much better than before W=512 opt.)
                (9, _) => [856, 4962, 188774, 296093, 377673, 426007, 462186],   // 9, 5.3, 4096, W=512, 1.48%
                (10, ..=512) => [-65892, 66203, 136361, 155795, 162095, 171627, 174716],    // 5.9
                (10, ..=1024) => [-68997, 73279, 129075, 155620, 172343, 188227, 193457],   // 5.9, W=512
                (10, ..=2048) => [-2383, 11093, 124723, 185117, 221925, 245415, 255824], // 10, 5.9, 2048, W=512
                (10, _) => [1264, -20731, 187118, 285868, 378503, 400347, 432024], // 10, 5.9, 4096, W=512, 1.15%
                ...
                (_, ..=2048) => [-1638, 10081, 112478, 166286, 195569, 225010, 233819], // 11, 6.5, 2048, W=512, 0.94%
                (11, _) => [2945, 13340, 219515, 311622, 397987, 434376, 459408], // 11, 6.5, 4096, W=512, 0.88%
                (_, _) => [2923, 14726, 245305, 344367, 399203, 439784, 461775] // 12, 7.1, 4096, W=512, 0.69%
            }
