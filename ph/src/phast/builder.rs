...
#[inline]
fn bucket_sizes_st(keys: &[u64], buckets_num: usize) -> Box<[usize]> {
    let mut buckets = vec![0; buckets_num + 1].into_boxed_slice();
    for key in keys.iter() { unsafe{ *buckets.get_unchecked_mut(bucket_for(*key, buckets_num)) += 1 } }
    buckets
}

fn bucket_sizes_mt(keys: &[u64], buckets_num: usize, mut threads_num: usize) -> Box<[usize]> { // TODO pass conf values to make fn non-generic
    //let mut threads_num = rayon::current_num_threads().min(keys.len() / (8*4096));
    threads_num = threads_num.min(keys.len() / (8*4096));
    if threads_num <= 1 { return bucket_sizes_st(keys, buckets_num); }
    let mut buckets = vec![0; buckets_num + 1].into_boxed_slice();
    ...
    while key_idx < keys.len() {
        ...
        if let Some(mut last_bucket_for_thread) = keys.get(new_key_idx).map(|key| bucket_for(*key, buckets_num)) {
            ...
            while new_key_idx < keys.len() {
                let b = bucket_for(keys[new_key_idx], buckets_num);
    ...
    threads.into_par_iter().for_each(|(keys, buckets, first)| {
        for key in keys.iter() {
            unsafe{ *buckets.get_unchecked_mut(bucket_for(*key, buckets_num)-first) += 1 };
        }
    });
    buckets
}
...
pub fn bucket_begin_st(keys: &[u64], buckets_num: usize) -> Box<[usize]> {
    let mut buckets = bucket_sizes_st(keys, buckets_num);
...
pub fn bucket_begin_mt(keys: &[u64], buckets_num: usize, threads_num: usize) -> Box<[usize]> {
    let mut buckets = bucket_sizes_mt(keys, buckets_num, threads_num);
    let sum = accumulative_sum(buckets.iter_mut()); //bucket_lens
    debug_assert_eq!(sum, keys.len());
    buckets
}
...
{
    let (builder, mut seeds) = BuildConf::new(keys, conf, seed_size, WINDOW_SIZE, evaluator, bucket_begin_st(keys, conf.buckets_num()), SeedOnlyNoBump);
...
{
    let (builder, mut seeds) = BuildConf::new(keys, conf, seed_size, WINDOW_SIZE, evaluator, bucket_begin_st(keys, conf.buckets_num()), seed_chooser);
...
where C: ConfTrait + Sync, SC: SeedChooser + Sync, BE: BucketToActivateEvaluator + Sync, BE::Value: Send, SS: SeedSize
{
    ...
    if threads_num == 1 {
        let (builder, mut seeds) = BuildConf::new(keys, conf, seed_size, span_limit, evaluator, bucket_begin_st(keys, conf.buckets_num()), seed_chooser);
        ThreadBuilder::<C, SC, _, _>::new(&builder, 0..conf.buckets_num(), 0, &mut seeds).build();
        //let (unassigned_values, unassigned_len) = builder.unassigned_values(&seeds);
        return (seeds, builder);
        //return build_st(keys, conf, span_limit, evaluator);
    }
    let bucket_begin = bucket_begin_mt(keys, conf.buckets_num(), threads_num);   // moving down makes program slower
    ...
    thread_builders.push(ThreadBuilder::<C, SC, _, _>::new(&builder, bucket_begin..conf.buckets_num(), 0, remaining_seeds));
    /*thread::scope(|s| {
        for thread in &mut thread_builders {
            s.spawn(|| thread.build());
        }
    });*/
    thread_builders.par_iter_mut().for_each(ThreadBuilder::<C, SC, _, _>::build);  // marginally faster than std
