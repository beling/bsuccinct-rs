...
pub fn benchmark_rank_select(conf: &super::Conf) {
    println!("bitm ArrayWithRankSelect101111:");
    let (content, tester) = build_bit_vec(conf);

    let (rs, _) = ArrayWithRankSelect101111::<BinaryRankSearch, BinaryRankSearch, _>::build(content);
    //assert_eq!(ones, conf.num);

    tester.raport_rank("bitm ArrayWithRankSelect101111",
    ...
    tester.raport_select1("bitm ArrayWithRankSelect101111 binary search over ranks",
            0.0, |index| rs.select(index));
    tester.raport_select0("bitm ArrayWithRankSelect101111 binary search over ranks",
    ...
    let (rs, _) = ArrayWithRankSelect101111::<CombinedSampling, CombinedSampling, _>::build(rs.content);

    /*println!(" checked select by combined sampling:");
    tester.raport_select1("bitm ArrayWithRankSelect101111 combined sampling",
            percent_of(rs.select_support().size_bytes(), rs.content.size_bytes()),
            |index| rs.select(index));
    tester.raport_select0("bitm ArrayWithRankSelect101111 combined sampling",
...
/*pub fn benchmark_rank_select(conf: &super::Conf) {
    ...
    /*println!(" checked select by combined sampling:");
        conf.raport_select1("bitm checked ArrayWithRankSelect101111 combined sampling",
            percent_of(rs.select_support().size_bytes(), rs.content.size_bytes()),
            |index| rs.select(index));
        conf.raport_select0("bitm checked ArrayWithRankSelect101111 combined sampling",
            percent_of(rs.select0_support().size_bytes(), rs.content.size_bytes()),
            |index| rs.select0(index));*/
