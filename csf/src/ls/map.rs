...
impl<S: BuildSeededHasher> Map<S> {
    ...
    #[inline(always)] fn try_with_vertex_t_conf_fn<VI, K, KBorrow, KVIntoIterator, FKVIntoIterator, BM>(
        keys: FKVIntoIterator, keys_len: usize, number_of_vertices: usize, third_of_vertices_len: usize,
              ...
              BM: BufferManager // buffer creator (and initializer)
    {
        for iteration in 0..100 {
            ...
            if queue.len() == keys_len {
                let mut rvalues = conf.buffer_manager.create(number_of_vertices, bits_per_value);
                for (index, (v0, v1, v2)) in queue.into_iter().enumerate().rev() {
                    ...
                    conf.buffer_manager.init(&mut rvalues, v0.to_usize(), value, bits_per_value);
                }
                return Some(Self {
                    values: rvalues,
    ...
    pub fn try_with_conf_fn<K, KBorrow, KVIntoIterator, FKVIntoIterator, BM>(
        keys: FKVIntoIterator, keys_len: usize, bits_per_value: u8, conf: MapConf<BM, S>
    ) -> Option<Self>
        where KVIntoIterator: IntoIterator<Item=(KBorrow, u64)>,    // Iterator over key-value pairs
              ...
              BM: BufferManager // buffer creator (and initializer)
    {
        if keys_len == 0 || bits_per_value == 0 { return Some(Self::always_map_to_zero(conf.hash_builder)); }
        // numer of keys = numbers of hyper-edges
        let mut third_of_vertices_len = ceiling_div(123 * keys_len, 300);
        ...
        if number_of_vertices <= 1>>32 {
            Self::try_with_vertex_t_conf_fn::<u32, K, _, _, _, _>(keys, keys_len, number_of_vertices, third_of_vertices_len, bits_per_value, conf)
        } else {
            Self::try_with_vertex_t_conf_fn::<usize, K, _, _, _, _>(keys, keys_len, number_of_vertices, third_of_vertices_len, bits_per_value, conf)
        }
    }
    ...
    where K: Hash, V: Into<u64> + Clone, BM: BufferManager {
    ...
    where K: Hash, V: Into<u64> + Clone + Ord, BM: BufferManager {
    ...
    /// Returns index of values fun_number-th fragment that is associated with given key k.
    #[inline(always)]
    fn index<K: Hash>(&self, k: &K, fun_number: u8) -> usize {
        (fun_number as usize * self.third_of_values_len) +
            index(&self.hash_builder, k, self.hash_seeds[fun_number as usize], self.third_of_values_len)
    }

    #[inline(always)]
    fn value_part<K: Hash>(&self, k: &K, fun_number: u8) -> u64 {
        self.values.get_fragment(self.index(k, fun_number), self.bits_per_value)
    }

    #[inline(always)]
    pub fn get<K: Hash>(&self, k: &K) -> u64 {
        self.value_part(k, 0) ^ self.value_part(k, 1) ^ self.value_part(k, 2)
    }
}
...
mod tests {
    ...
    fn lsmap_1bpv_conf<BM: BufferManager>(conf: MapConf<BM>) {
    ...
    fn lsmap_2bpv_conf<BM: BufferManager>(conf: MapConf<BM>) {
    ...
    fn lsmap_3bpv_conf<BM: BufferManager>(conf: MapConf<BM>) {
