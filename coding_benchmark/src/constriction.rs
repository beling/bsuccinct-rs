...
pub fn benchmark(conf: &super::Conf) {
    ...
    conf.print_speed("Encoding without adding to bit vector (prefix order)", conf.measure(|| { // (symbol prefix order)
        for k in text.iter() {
    ...
    conf.print_speed("Encoding + adding to bit vector (prefix order)", conf.measure(|| encode_prefix(&text, &encoder_codebook)));
    conf.print_speed("Encoding + adding to bit vector (suffix order)", conf.measure(|| encode_suffix(&text, &encoder_codebook)));

    let cursor = Cursor::new_at_write_beginning(encode_prefix(&text, &encoder_codebook));
    conf.print_speed("Decoding from a queue (prefix order) (without storing)", conf.measure(|| {
