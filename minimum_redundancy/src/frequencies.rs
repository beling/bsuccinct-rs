...
impl Weight for usize {
    ...
    #[inline(always)] fn from(value: u32) -> Self { value as Self }
    //#[inline(always)] fn set_to(&mut self, value: u32) { *self = value as Self; }
}
...
impl Weight for u32 {
    #[inline(always)] fn as_usize(self) -> usize { self as usize }
    #[inline(always)] fn as_f64(self) -> f64 { self as f64 }
    #[inline(always)] fn from(value: u32) -> Self { value }
    //#[inline(always)] fn set_to(&mut self, value: u32) { *self = value as Self; }
}
...
impl<Value: Eq + Hash, W: Weight, S: BuildHasher + Default> Frequencies for HashMap<Value, W, S> {
    ...
    #[inline(always)] fn occurrences_of(&mut self, value: &Self::Value) -> Self::Weight {
        self.get(value).map_or(Self::Weight::from(0), |v| *v)
    }

    #[inline(always)] fn number_of_occurring_values(&self) -> usize { self.len() }

    #[inline(always)] fn drain_frequencies(&mut self) -> impl Iterator<Item=(Self::Value, Self::Weight)> { HashMap::drain(self) }
    ...
    #[inline(always)] fn add_occurrence_of(&mut self, value: Value) {
        *self.entry(value).or_insert(Self::Weight::from(0)) += Self::Weight::from(1);
    }
...
impl<W: Weight> Frequencies for [W; 256] {
    ...
    #[inline(always)] fn add_occurrence_of(&mut self, value: Self::Value) {
        self[value as usize] += Self::Weight::from(1);
    }

    #[inline(always)] fn number_of_occurring_values(&self) -> usize {
        self.iter().filter(|occ| **occ > Self::Weight::from(0)).count()
    }
    ...
    #[inline(always)] fn frequencies(&self) -> impl Iterator<Item=(Self::Value, Self::Weight)> where Self::Value: Clone {
        self.iter().enumerate().filter_map(|(v, o)| (*o > Self::Weight::from(0)).then(|| (v as u8, *o)))
    }

    #[inline(always)] fn occurrences(&self) -> impl Iterator<Item = Self::Weight> {
        self.iter().filter_map(|occ| (*occ > Self::Weight::from(0)).then_some(*occ))
    }

    #[inline(always)] fn without_occurrences() -> Self { [Self::Weight::from(0); 256] }
}